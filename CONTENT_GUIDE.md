...
To add a course with a title, use an object:
...
For each term, `term` and `courses` are required. `id` is optional; the site derives it from the term name. Use `status: "completed"` for finished terms and `status: "planned"` for future terms. Add `coop: true` when a term carries a co-op work term. (The site intentionally does not display grades or GPA.)

## Analytics

Recruiter signal is tracked with Vercel Analytics custom events (`assistant_question`,
`project_view`) — view them in the Vercel dashboard once deployed. Wired in
`components/PortfolioAgent.tsx` and `components/TrackView.tsx` (on case-study pages)
via `lib/analytics.ts`.
